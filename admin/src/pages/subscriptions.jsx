...
export function SubscriptionsPage() {
  ...
  // Upgrade Requests (Pending Approvals)
  ...
  const pendingRequests = useMemo(() => {
    return subscribers.filter((s) => s.status === "pending");
  }, [subscribers]);

  const handleApproveSub = async (subId) => {
    try {
      setProcessingSubId(subId);
      const res = await api.patch(`/admin/subscriptions/${subId}/approve`);
      toast.success(res.data?.message || "Subscription upgrade approved successfully!");
      fetchData();
    } catch (err) {
      console.error(err);
      toast.error(err.response?.data?.message || "Failed to approve subscription upgrade.");
    } finally {
  ...
  const handleConfirmRejectSub = async () => {
    ...
    try {
      setProcessingSubId(rejectingSub._id);
      const res = await api.patch(`/admin/subscriptions/${rejectingSub._id}/reject`, {
        reason: rejectReason.trim() || undefined,
      });
      toast.success(res.data?.message || "Subscription upgrade request rejected.");
      ...
      console.error(err);
      toast.error(err.response?.data?.message || "Failed to reject subscription upgrade.");
    } finally {
      setProcessingSubId(null);
    }
  };
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {/* TABS CONTENT */}
      {activeTab === "requests" && (() => {
        const filteredRequests = subscribers.filter((sub) => {
          ...
          return u.includes(q) || r.includes(q) || p.includes(q) || e.includes(q);
        });

        const totalReqs = filteredRequests.length;
        const totalReqPages = Math.max(1, Math.ceil(totalReqs / reqItemsPerPage));
        const safeReqPage = Math.min(reqPage, totalReqPages);
        const startIndex = (safeReqPage - 1) * reqItemsPerPage;
        const paginatedReqs = filteredRequests.slice(startIndex, startIndex + reqItemsPerPage);
          ...
          <div className="space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div>
                <h3 className="text-sm font-semibold flex items-center gap-2">
                  ...
                  {totalReqs > 0 && (
                    <Badge variant="secondary" className="rounded-full px-2 py-0.5 text-xs font-bold text-amber-500 bg-amber-500/10 border-0">
                      {totalReqs} Pending
                    </Badge>
                  )}
                </h3>
                <p className="text-xs text-muted-foreground mt-0.5">
                  Review and approve creator/brand subscription upgrade requests. Once approved, the plan activates immediately.
                </p>
              </div>
              ...
              <div className="flex items-center gap-2">
                <div className="relative w-full sm:w-72">
                  ...
                  <Input
                    placeholder="Search user, role, requested plan..."
                    value={reqSearch}
                    onChange={(e) => {
                ...
                <Button
                  ...
                  size="sm"
                  onClick={fetchData}
                  className="rounded-full h-9 px-3 text-xs flex items-center gap-1.5"
                >
            ...
            <div className="rounded-2xl border border-border bg-card overflow-hidden">
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead className="pl-6">User / Account</TableHead>
                    <TableHead>Account Type</TableHead>
                    <TableHead>Requested Plan</TableHead>
                    <TableHead>Price / Billing</TableHead>
                    <TableHead>Requested At</TableHead>
                    <TableHead>Status</TableHead>
                    <TableHead className="text-right pr-6">Review Actions</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {paginatedReqs.map((sub) => {
                    const isProcessing = processingSubId === sub._id;
                    return (
                      <TableRow key={sub._id} className="hover:bg-muted/40 transition-colors">
                          ...
                          <div className="flex items-center gap-3">
                            {sub.avatarUrl ? (
                              ...
                              <div className="h-9 w-9 rounded-full bg-primary/10 text-primary flex items-center justify-center font-bold text-xs shrink-0">
                                {(sub.user || "U").slice(0, 2).toUpperCase()}
                              </div>
                            )}
                            <div>
                              <p className="font-semibold text-foreground text-xs leading-tight">{sub.user}</p>
                              <p className="text-[11px] text-muted-foreground">{sub.email || sub.handle || "No email"}</p>
                            </div>
                          </div>
                        </TableCell>
                        ...
                        <TableCell>
                          <Badge
                            variant="outline"
                            className={`rounded-full text-[10px] uppercase font-bold border-0 px-2 py-0.5 ${
                              sub.role === "brand"
                        ...
                        <TableCell>
                          <span className="font-semibold text-primary text-xs">
                            {sub.currentPlan || sub.packageName || "Package"}
                          </span>
                        </TableCell>

                        <TableCell className="text-xs">
                          {sub.packagePrice !== undefined ? (
                            <span className="font-medium text-foreground">
                              ₹{sub.packagePrice.toLocaleString("en-IN")}
                        ...
                        <TableCell>
                          <Badge
                            variant="outline"
                            className="rounded-full text-[10px] font-bold bg-amber-500/10 text-amber-500 border-0 flex items-center gap-1 w-fit"
                          >
                        ...
                        <TableCell className="text-right pr-6">
                          <div className="flex items-center justify-end gap-2">
                            <Button
                              ...
                              className="h-8 px-3 rounded-full text-xs font-semibold bg-emerald-600 hover:bg-emerald-700 text-white flex items-center gap-1 shadow-xs"
                            >
                            ...
                            <Button
                              ...
                              className="h-8 px-3 rounded-full text-xs font-semibold border-rose-500/30 text-rose-500 hover:bg-rose-500/10 flex items-center gap-1"
                            >
                    ...
                    <TableRow>
                      <TableCell colSpan={7} className="text-center py-16">
                        <div className="flex flex-col items-center justify-center space-y-2 text-muted-foreground">
                          ...
                          <p className="font-semibold text-sm text-foreground">No Pending Upgrade Requests</p>
                          <p className="text-xs">
                            {reqSearch
                              ? "No requests match your current search query."
                              : "All creator and brand upgrade requests have been reviewed."}
                          </p>
                        </div>
                      </TableCell>
                    </TableRow>
            ...
            {totalReqs > 0 && (
              <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-4 border-t border-border/40 text-xs">
                <span className="text-muted-foreground order-2 sm:order-1">
                  ...
                  <strong className="text-foreground">{Math.min(safeReqPage * reqItemsPerPage, totalReqs)}</strong> of{" "}
                  <strong className="text-foreground">{totalReqs}</strong> requests
                </span>
                {totalReqPages > 1 && (
                  <div className="flex items-center gap-1.5 order-1 sm:order-2">
                    ...
                    <div className="flex items-center gap-1">
                      {getPageNumbers(safeReqPage, totalReqPages).map((page, idx) =>
                        page === "..." ? (
                          <span key={`ellipsis-req-${idx}`} className="px-1 text-muted-foreground">
                            ...
                          </span>
                        ) : (
                          <Button
                            key={page}
                            variant={safeReqPage === page ? "default" : "outline"}
                            size="sm"
      ...
      <Dialog open={rejectDialogOpen} onOpenChange={setRejectDialogOpen}>
        <DialogContent className="max-w-md rounded-2xl p-6">
          <DialogHeader>
            ...
            <DialogDescription className="text-xs text-muted-foreground">
              Decline the upgrade request for{" "}
              <strong className="text-foreground">{rejectingSub?.user}</strong> (
              {rejectingSub?.currentPlan || rejectingSub?.packageName}).
            </DialogDescription>
          </DialogHeader>

          <div className="space-y-3 py-2">
            <Label htmlFor="rejectReason" className="text-xs font-semibold">
              Reason for Declining (optional, will be notified to user)
            </Label>
            <Textarea
              id="rejectReason"
              value={rejectReason}
              onChange={(e) => setRejectReason(e.target.value)}
              placeholder="e.g. Incomplete payment, requirement not met, or please contact support..."
              rows={3}
          ...
          <DialogFooter className="pt-2 flex items-center justify-end gap-2">
            ...
            <Button
              ...
              variant="destructive"
              size="sm"
              disabled={processingSubId !== null}
              className="rounded-full text-xs font-semibold"
              onClick={handleConfirmRejectSub}
            >
              {processingSubId === rejectingSub?._id ? "Rejecting..." : "Decline Request"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
